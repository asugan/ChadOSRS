...
def test_load_config_and_build_runelite_http_with_action_runner(tmp_path: Path) -> None:
    config_path = tmp_path / "runelite_http_actions.json"
    config_path.write_text(
        """
{
  "adapter_mode": "runelite_http",
  ...
  "runelite_http": {
    ...
    "target_pos": [3200, 3200],
    "obstacles": [],
    "enable_action_runner": true,
    "action_url": "http://127.0.0.1:8766/action",
    "action_timeout_s": 1.2,
    "action_auth_token": "abc"
  }
}
""".strip(),
        encoding="utf-8",
    ...
    try:
        assert isinstance(perception, RuneLitePerception)
        assert isinstance(runner, RuneLiteHttpActionRunner)
        assert app_config.runelite_http.enable_action_runner is True
        assert app_config.runelite_http.action_url == "http://127.0.0.1:8766/action"
    finally:
        if hasattr(perception, "close"):
            getattr(perception, "close")()
